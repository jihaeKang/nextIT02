...
function p_ctrlFiles(){
	...
	// click file button on input file
	$(".ctrlFile .ctrlFile-btnFile").click(function (event) {
		$(this).closest(".ctrlFile").find("input[type=file]").click();
	});

	// change input file field
	$(".ctrlFile input[type=file]").change(function (event) {
		var $this = $(this);
		...
		$this.parent().find(".ctrlFile-fileTxt").text(fileVal[fileValLength-1]);
		$this.parent().find(".ctrlFile-btnDel").show();
	});

	// delete
	$(".ctrlFile input[name='delBtn']").click(function (event) {
		var $this = $(this);
		$this.parent().find("input[type=file]").val('');
		$this.parent().find(".ctrlFile-fileTxt").text(placeTxt);
		$this.hide();
	});
}
// 컨트롤 개인정보 방침 컨틀롤
